...
test('applyFix: verified cells are dated, failures are demoted with their old data kept in notes, errors are untouched', () => {
  ...
  const reports: CellReport[] = [
    ...
    { app: 'a', question: 'gone', value: 'partial', status: 'fail', method: 'none', evidence_url: 'https://docs.example/page', problems: ['quote not found on page'] },
...
test('applyFix twice is stable: a demoted cell is skipped next time instead of failing again', () => {
  const first = applyFix([cell('a', 'gone', 'yes')], [{ app: 'a', question: 'gone', value: 'yes', status: 'fail', method: 'none', evidence_url: 'https://docs.example/page', problems: ['quote not found on page'] }], [], '2026-09-10');
